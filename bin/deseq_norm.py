...
def median_normalization(density_vectors,threshold = 0):
    gm_means = np.zeros(len(density_vectors[density_vectors.keys()[0]]))
    for d in density_vectors: gm_means += np.log(density_vectors[d])
    gm_means /= float(len(density_vectors))

    norm_vectors = {}
    good = np.exp(gm_means) > threshold
    for d in density_vectors:
        s = np.median(gm_means[good]-np.log(density_vectors[d][good]))
        norm_vectors[d] = density_vectors[d] * np.exp(s)
    return norm_vectors

def quantile_normalization(density_vectors,base_key = None):
    if base_key is None: base_key = density_vectors.keys()[0]

    values = np.sort(density_vectors[base_key])
    print values[-10:]
    normed_vectors = {c:np.zeros(len(density_vectors[c])) for c in density_vectors}
    for key in density_vectors:
        order = np.argsort(density_vectors[key])
        normed_vectors[key][order] = values
    return normed_vectors


def median_size_factors(density_matrix,threshold = 0):
    gm_means = np.mean(np.log(density_matrix.values),axis = 1)

    good = np.exp(gm_means) > threshold
    size_factors = {}
    for d in density_matrix.columns.values:
        s = np.median(gm_means[good]-np.log(density_matrix[d].values[good]))
        size_factors[d] = 1/np.exp(s)
    return size_factors
...
def scale_vectors(density_vectors):
    ...
    return {d:density_vectors[d]*overall_avg/float(mean_values[d]) for d in mean_values}
...
def loess_geomean_norm(density_vectors, called_vectors, sample_number=SAMPLE_NUMBER,sample_method = 'random',
                        correlation_limit = CORRELATION_LIMIT,
                        cv_number = 5,pre_normalize = True,constitutive = False):

    if pre_normalize:
        size_normed = scale_vectors(density_vectors)
    else:
        size_normed = density_vectors
    print [(d,np.mean(size_normed[d])) for d in size_normed]
    ...
    if constitutive:
        ...
        decent_peaks = np.where(numbers==np.max(numbers))[0]
    else:
        decent_peaks = np.arange(len(avg_values)).astype(int)
