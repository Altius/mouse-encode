...
from stamlims_api.lims import libraries
...
def get_sample_data(sample_id):
    js_data = api.get_single_result(url_addition='sample/?numbers=%d' %(sample_id))
    js_data = json.loads(json.dumps(js_data))['results'][0]
    return js_data
...
def get_ds_aggregations(ds_id,pg_size = 100):
    aggregations = []
    ds_ids = []
    js_data = api.get_single_result(url_addition='library/?numbers=%d&page_size=%d' %(ds_id,pg_size))
    js_data = json.loads(json.dumps(js_data))
    aggregations = [x['id'] for x in js_data['results']]
    ds_ids = ['DS'+str(x['sample_number'])+x['library_sublibrary'] for x in js_data['results']]
    while js_data['next'] is not None:
...
def get_ds_libraries(ds_id,pg_size = 100):
    libraries = lims.libraries.list_libraries(api,ds_id)
    output = []
    for lb in libraries:
        nm = 'DS%d%s' %(ds_id,lb['sub_library'])
        if lb['kit_method'] is None:
            method = None
        else:
            method = parse_url(lb['kit_method'].split('api/')[1])['name']
        output.append((nm,method,lb['finished_date']))
    return output

def parse_url(kit_url):
    print kit_url
    js_data = api.get_single_result(url_addition=kit_url)
    js_data = json.loads(json.dumps(js_data))
    return js_data
